...
/**
 * Responsive details page layout with top summary, KPI grid,
 * date picker area, and a placeholder chart.
 */
const DataUsageDetails: React.FC<Props> = ({ row, onBack }) => {
  const name = (row.name as string) ?? (row.client as string) ?? 'Device'
  const dataUsage =
    row.dataUsage && typeof row.dataUsage === 'string'
      ? row.dataUsage
      : row.dataUsage && typeof row.dataUsage !== 'string'
      ? null
      : '—'
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <button
            onClick={onBack}
            className="text-sm underline text-muted-foreground mb-2"
          >
            ← Back
          </button>
        ...
        <div className="h-48 sm:h-64 w-full bg-gradient-to-r from-white to-gray-50 rounded-lg flex items-center justify-center text-muted-foreground">
          ...
          >
            ...
            <polyline
              ...
              stroke="#3b82f6"
              strokeWidth="3"
              points="10,240 80,200 160,220 240,160 320,170 400,150 480,180 560,140 640,160 720,120"
            />
            <polyline
              fill="none"
              stroke="#f59e0b"
              strokeWidth="3"
              points="10,200 80,160 160,180 240,140 320,200 400,220 480,200 560,170 640,190 720,170"
            />
          </svg>
        </div>
      </div>
